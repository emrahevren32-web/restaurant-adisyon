export type BusinessWorkspaceRoute =
  | 'workspace-welcome'
  | 'tables'
  | 'products'
  // Aşama 2 · Depo çekirdeği. Gerçek veritabanına konuşan ilk iş ekranı;
  // 'stock-cards'/'stock-movements' hâlâ localStorage üzerindedir (ADR-003
  // "dikey dilim": yenisi yanına kurulur, eskisi yerinde kalır).
  | 'depo'
  | 'tedarikciler'
  | 'satinalma'
  | 'receteler'
  | 'uretim-emirleri'
  | 'izlenebilirlik'
  | 'sevkiyatlar'
  | 'haccp-kayitlari'
  | 'sayimlar'
  | 'zayi-imha'
  | 'islem-gecmisi'
  | 'veri-yedegi'
  | 'stock-cards'
  | 'stock-movements'
  | 'inventory-lots'
  | 'chemical-products'
  | 'shipment-work-orders'
  | 'shipment-pallets'
  | 'shipment-vehicles'
  | 'shipment-plans'
  | 'shipment-forms'
  | 'delivery-notes'
  | 'shipment-returns'
  | 'shipment-waybills'
  | 'shipments'
  | 'shipment-executions'
  | 'transfer-receipts'
  | 'production-work-orders'
  | 'production-planning'
  | 'capacity-planning'
  | 'machine-scheduling'
  | 'workforce-planning'
  | 'bottleneck-analysis'
  | 'continuous-improvement'
  | 'production-lines'
  | 'intermediate-products'
  | 'final-products'
  | 'blast-chiller-processes'
  | 'packaging-processes'
  | 'labeling-processes'
  | 'dispatch-processes'
  | 'recipes'
  | 'fire-analysis'
  | 'cost-engine'
  | 'purchase-requests'
  | 'request-for-quotations'
  | 'purchase-approvals'
  | 'purchase-orders'
  | 'goods-receipts'
  | 'supplier-performances'
  | 'procurement-analytics'
  | 'kpi-dashboard'
  | 'daily-production-analytics'
  | 'weekly-production-analytics'
  | 'shipment-analytics'
  | 'waste-analytics'
  | 'cost-analytics'
  | 'warehouse-performance-analytics'
  | 'personnel-performance-analytics'
  | 'decision-support'
  | 'critical-alerts'
  | 'forecasting'
  | 'recommendation-engine'
  | 'ai-analysis'
  | 'cost-optimization'
  | 'purchase-recommendations'
  | 'production-planning-recommendations'
  | 'waste-predictions'
  | 'shipment-optimization'
  | 'label-management'
  | 'operation-checklists'
  | 'quality-controls'
  | 'quality-control-forms'
  | 'waste-management'
  | 'lot-system'
  | 'sample-tracking'
  | 'witness-samples'
  | 'product-recalls'
  | 'product-history'
  | 'haccp-management'
  | 'return-processes'
  | 'supplier-returns'
  | 'suppliers'
  | 'supplier-debts'
  | 'supplier-payments'
  | 'cash-transactions'
  | 'income-expense'
  | 'cash-closing'
  | 'financial-reports'
  | 'cash-transfers'
  | 'business-summary'
  | 'sales-revenue-analysis'
  | 'product-performance-analysis'
  | 'stock-risk-center'
  | 'current-finance-center'
  | 'personnel-performance-center'
  | 'manager-alert-center'
  | 'summary'
  | 'history'
  | 'kitchen'
  | 'qr-orders'
  | 'qr-codes'
  | 'actions'
  | 'employee-cards'
  | 'shift-management'
  | 'attendance-tracking'
  | 'employee-performance'
  | 'employee-bonus'
  | 'employee-audit'
  | 'employee-reports'
  | 'staff'
  | 'current-report'
  | 'risky-current'
  | 'users'
  | 'branches'
  | 'branch-permissions'
  | 'branch-reporting'
  | 'branch-stock-transfers'
  | 'head-office-management'
  | 'current-accounts'
  | 'credit-transactions'
  | 'collection-transactions'
  | 'current-account-movements'
  | 'settings'
  | 'company-profile'
  | 'my-profile'
  | 'notifications'
  | 'marketplace'
  | 'integration-center'
  | 'excel-center'

export type PlatformRoute =
  | 'business-registration-system'
  | 'company-setup-wizard'
  | 'package-license-management'
  | 'user-subscription-management'
  | 'module-activation-system'
  | 'tenant-management'
  | 'evren360-dashboard'
  | 'evren360-customer-list'
  | 'evren360-customer-detail'
  | 'evren360-pending-applications'
  | 'evren360-license-management'
  | 'evren360-system-announcements'
  | 'evren360-customer-statistics'
  | 'evren360-company-management'
  | 'evren360-billing-management'
  | 'evren360-sector-management'
  | 'evren360-applications'
  | 'evren360-companies'
  | 'evren360-packages'
  | 'evren360-modules'
  | 'evren360-licenses'
  | 'evren360-subscriptions'
  | 'evren360-users'
  | 'evren360-support'
  | 'evren360-stats'
  | 'evren360-settings'

export type AppRoute = BusinessWorkspaceRoute | PlatformRoute

export type BusinessWorkspaceNavKey =
  | 'workspace-welcome'
  | 'dashboard'
  | 'workspace'
  | 'business-summary'
  | 'users'
  | 'roles'
  | 'branches'
  | 'notifications'
  | 'license'
  | 'subscription'
  | 'settings'
  | 'company-profile'
  | 'my-profile'
  | 'marketplace'
  | 'integration-center'
  | 'excel-center'
  | 'support'
  | 'ai-center'
  | 'action-history'
  | 'adisyon'
  | 'tables-management'
  | 'products'
  | 'kitchen'
  | 'bill-history'
  | 'qr-orders'
  | 'waiter-calls'
  | 'qr-codes'
  | 'depo'
  | 'tedarikciler'
  | 'satinalma'
  | 'receteler'
  | 'uretim-emirleri'
  | 'izlenebilirlik'
  | 'sevkiyatlar'
  | 'haccp-kayitlari'
  | 'sayimlar'
  | 'zayi-imha'
  | 'islem-gecmisi'
  | 'veri-yedegi'
  | 'stock-cards'
  | 'stock-movements'
  | 'critical-stock'
  | 'expiry-lots'
  | 'inventory-lots'
  | 'chemical-products'
  | 'shipment-work-orders'
  | 'shipment-pallets'
  | 'shipment-vehicles'
  | 'shipment-plans'
  | 'shipment-forms'
  | 'delivery-notes'
  | 'shipment-returns'
  | 'shipment-waybills'
  | 'shipments'
  | 'shipment-executions'
  | 'transfer-receipts'
  | 'waste'
  | 'production-work-orders'
  | 'production-planning'
  | 'capacity-planning'
  | 'machine-scheduling'
  | 'workforce-planning'
  | 'bottleneck-analysis'
  | 'continuous-improvement'
  | 'production-lines'
  | 'intermediate-products'
  | 'final-products'
  | 'blast-chiller-processes'
  | 'packaging-processes'
  | 'labeling-processes'
  | 'dispatch-processes'
  | 'recipes'
  | 'fire-analysis'
  | 'cost-engine'
  | 'purchase-requests'
  | 'request-for-quotations'
  | 'purchase-approvals'
  | 'purchase-orders'
  | 'goods-receipts'
  | 'supplier-performances'
  | 'procurement-analytics'
  | 'kpi-dashboard'
  | 'daily-production-analytics'
  | 'weekly-production-analytics'
  | 'shipment-analytics'
  | 'waste-analytics'
  | 'cost-analytics'
  | 'warehouse-performance-analytics'
  | 'personnel-performance-analytics'
  | 'decision-support'
  | 'critical-alerts'
  | 'forecasting'
  | 'recommendation-engine'
  | 'ai-analysis'
  | 'cost-optimization'
  | 'purchase-recommendations'
  | 'production-planning-recommendations'
  | 'waste-predictions'
  | 'shipment-optimization'
  | 'label-management'
  | 'operation-checklists'
  | 'quality-controls'
  | 'quality-control-forms'
  | 'waste-management'
  | 'lot-system'
  | 'sample-tracking'
  | 'witness-samples'
  | 'product-recalls'
  | 'product-history'
  | 'haccp-management'
  | 'return-processes'
  | 'supplier-returns'
  | 'suppliers'
  | 'current-accounts'
  | 'current-account-movements'
  | 'current-report'
  | 'risky-current'
  | 'current-finance-center'
  | 'credit-transactions'
  | 'collection-transactions'
  | 'supplier-debts'
  | 'supplier-payments'
  | 'cash-transactions'
  | 'income-expense'
  | 'cash-closing'
  | 'financial-reports'
  | 'cash-transfers'
  | 'employee-cards'
  | 'shift-management'
  | 'attendance-tracking'
  | 'employee-performance'
  | 'employee-bonus'
  | 'employee-audit'
  | 'employee-reports'
  | 'staff'
  | 'branch-directory'
  | 'branch-permissions'
  | 'branch-reporting'
  | 'branch-stock-transfers'
  | 'head-office-management'
  | 'sales-revenue-analysis'
  | 'product-performance-analysis'
  | 'stock-risk-center'
  | 'manager-alert-center'
  | (string & {})

export type PlatformNavKey =
  | 'business-registration-system'
  | 'company-setup-wizard'
  | 'package-license-management'
  | 'user-subscription-management'
  | 'module-activation-system'
  | 'tenant-management'
  | 'evren360-dashboard'
  | 'evren360-customer-list'
  | 'evren360-customer-detail'
  | 'evren360-pending-applications'
  | 'evren360-license-management'
  | 'evren360-system-announcements'
  | 'evren360-customer-statistics'
  | 'evren360-company-management'
  | 'evren360-billing-management'
  | 'evren360-sector-management'
  | 'evren360-applications'
  | 'evren360-companies'
  | 'evren360-packages'
  | 'evren360-modules'
  | 'evren360-licenses'
  | 'evren360-subscriptions'
  | 'evren360-users'
  | 'evren360-support'
  | 'evren360-stats'
  | 'evren360-settings'

export type AppNavKey = BusinessWorkspaceNavKey | PlatformNavKey

export type BusinessWorkspaceNavGroupKey =
  | 'system-modules'
  | 'operations-modules'
  | 'business-modules'
  | 'report-modules'
  | 'yonetim-modules'
  | 'integration-modules'
export type PlatformNavGroupKey = 'evren360-admin'
export type AppNavGroupKey = BusinessWorkspaceNavGroupKey | PlatformNavGroupKey
