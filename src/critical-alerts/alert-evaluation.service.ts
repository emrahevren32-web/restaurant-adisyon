...
const createHaccpAlerts = (
  ...
  return withRule('alert-rule-haccp-fail', rule => flattenHACCPMonitoringRecords(input.sourceData.haccpRecords)
    ...
    .map(record => createAlert({
      ...
      rule,
      level: 'CRITICAL',
      reason: `HACCP monitoring FAIL: olculen ${record.measuredValue}, limit ${record.criticalLimit}.`,
...
const createQualityAlerts = (
  ...
  const qualityAlerts = withRule('alert-rule-quality-fail', rule => QualityFormService.list(input.sourceData)
    ...
    .map(form => createAlert({
      ...
      reason: `${form.formNo} kalite formunda FAIL sonucu veya kriteri tespit edildi.`,
      recommendedAction: 'Kalite FAIL orani kritik seviyeye ulasti; lot, urun ve supplier etkisi manuel incelenmeli.',
  ...
  const checklistAlerts = withRule('alert-rule-quality-fail', rule => ChecklistService.list(input.sourceData)
    ...
    .map(record => createAlert({
      ...
      category: 'QUALITY',
      title: 'Operations checklist FAIL',
      reason: `${record.checklistNo} checklist ${record.execution.failCount} FAIL ve ${record.execution.completionRate}% tamamlanma ile alarm uretir.`,
...
const createShipmentAndReceiptAlerts = (
  ...
  const receiptAlerts = withRule('alert-rule-goods-receipt-reject', rule => GoodsReceiptService.list(input.sourceData)
    ...
    .map(receipt => createAlert({
      ...
      recommendedAction: 'Mal kabul red riski supplier, kalite ve satin alma tarafinda manuel incelenmeli.',
      expectedImpact: 'Uygunsuz hammadde ve supplier riskini azaltir.',
