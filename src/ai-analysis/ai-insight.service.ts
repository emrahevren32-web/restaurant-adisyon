...
const createDecisionSupportInsights = (
  ...
  .map(suggestion => createInsight({
    ...
    suggestedPromptContext: `Karar destek kurali=${suggestion.ruleId}; risk=${suggestion.risk}; oncelik=${suggestion.priority}; durum=${suggestion.status}.`,
...
const createRecommendationInsights = (
  ...
  .map(item => createInsight({
    ...
    suggestedPromptContext: `Oneri kaynagi=${item.sourceModule}; risk=${item.risk}; oncelik=${item.priority}; guven=${formatNumber(item.confidenceScore, 1)}.`,
