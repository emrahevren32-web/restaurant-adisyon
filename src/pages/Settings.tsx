...
export default function Settings({ currentUser, onSettingsChange }: Props){
  ...
  const generateDemoData = () => {
    ...
    setMessage({
      type: 'success',
      text: `Demo veri oluşturuldu: ${demo.tables.length} masa, ${demo.categories.length} kategori, ${demo.products.length} ürün, ${demo.employees.length} personel, ${demo.shifts.length} vardiya, ${demo.currentAccounts.length} cari, ${demo.creditTransactions.length} veresiye kaydı, ${demo.collectionTransactions.length} tahsilat, ${demo.supplierDebts.length} tedarikçi borcu, ${demo.supplierPayments.length} tedarikçi ödemesi, ${demo.cashTransactions.length} manuel kasa hareketi, ${demo.incomeExpenses.length} gelir gider kaydı, ${demo.cashClosings.length} gün sonu kapanışı, ${demo.cashTransfers.length} kasa devri.`
    })
  }
